...
################################################################################
# Algorithm = ['mean_field', 'joint_actions', 'actor_critic']
# rewardsHistory = [[], [], []]
# rewardsMean = [[], [], []]
# Iters = 100
...
# plt.title('The Mean Reward of One Situation of An Inventory Game with {} Players'.format(NumOfPlayers))
# plt.xlabel('t')
    ...
    for exp in range(experiments_times):
        ...
        for step in range(Steps):
            ...
            for iters in range(Iters):
                ...
                rewards = env.__procudure__()
                env.record_flag = False
                if np.sum(np.array(rewards) - last_rewards) < 1e-4:
